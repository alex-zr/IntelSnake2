...
//        int headArea = BiggerArea.calcVertices(headVertex);
        ...
        if (left != null && left.point.itsMe(applePoint)) {
...
//                .max(Comparator.comparingInt(v -> BiggerArea.calcVertices(v, vertices)))
                .max((v1, v2) -> {
                    if (BiggerArea.calcVertices(v1, vertices) == BiggerArea.calcVertices(v2, vertices)) {
                        return Double.compare(applePoint.distance(v1.point), applePoint.distance(v2.point));
                    }
                    if (BiggerArea.calcVertices(v1, vertices) > BiggerArea.calcVertices(v2, vertices)) {
                        return 1;
                    }
    ...
    public static int calcVertices(Dijkstra.Vertex start, Dijkstra.Vertex[][] vertices) {
        if (start == null) {
            return 0;
        }
